    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
                        ...
                        with slim.arg_scope(
                            ...
                            with slim.arg_scope([slim.conv2d, slim.conv2d_in_plane,
                                ...
                                outputs = faster_rcnn.build_whole_detection_network(input_img_batch=img,
                                                                                    ...
                                                                                    gpu_id=i)
                                #rois_0, rois_1, lables_0, bbox_targets_0 = outputs[-4:]
        ...
        with tf.Session(config=tfconfig) as sess:
            ...
            for step in range(cfgs.MAX_ITERATION // num_gpu):
                ...
                #wat = sess.run([rois_0, rois_1, lables_0, bbox_targets_0, gtboxes_and_label_batch[i]])
                #if len(wat[4]) > 0:
                #    print('*!'*500)
                
                #print('gtboxes_and_label_batch[i]')
                #print(type(wat[4]))
                #print(wat[4].shape)
                #print(wat[4])
                ...
                if (global_stepnp-1)*num_gpu >= cfgs.MAX_ITERATION:
                    coord.request_stop()
                    coord.join(threads)
