...
#import logging
#logging.basicConfig(filename='XXbuiXld_whXole_network.log',level=logging.INFO)
...
#                              'bridge': 0.1, 'basketball-court': 0.3, 'baseball-diamond': 0.3, 'container-crane': 0.3}
                ...
                # threshold = {'TEL':0.3,'Radar-TET':0.3,'Radar-Vehicle':0.3}
    ...
    def build_whole_detection_network(self, input_img_batch, gtboxes_batch, gtboxes_r_batch, gpu_id):
        ...
        if self.is_training:
            # check if gtboxes_batch is empty
            with tf.variable_scope('sample_anchors_minibatch'):
                # This needs to be conditional for empty images, ie., empty gtboxes_batch.
                # What is the type/format of fpn_labels, fpn_bbox_targets? Can they be empty?
#                 fpn_labels, fpn_bbox_targets, wth, overlaps = \
#                     tf.py_func(
#                         anchor_target_layer,
...
#                         [tf.float32, tf.float32, tf.float32, tf.float32])
        ...
        final_bbox, final_scores, final_category = self.postprocess_fastrcnn(rois=rois,
                                                                             ...
                                                                             gpu_id=gpu_id)
        if self.is_training:
            return final_bbox, final_scores, final_category, self.loss_dict #, rois_0, rois_1, lables_0, bbox_targets_0
        else:
