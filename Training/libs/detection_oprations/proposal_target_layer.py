...
def proposal_target_layer(rpn_rois, gt_boxes_h, gt_boxes_r):
    ...
    return rois, labels.astype(np.float32), bbox_targets_r
...
def _sample_rois(all_rois, gt_boxes_h, gt_boxes_r, fg_rois_per_image,
    ...
    """
    ## johns edits starting here
    if len(gt_boxes_h) > 0: ### johns edit and tabbed below
    
        # overlaps: (rois x gt_boxes)
        overlaps = bbox_overlaps(
            np.ascontiguousarray(all_rois, dtype=np.float),
            np.ascontiguousarray(gt_boxes_h[:, :-1], dtype=np.float))
        gt_assignment = overlaps.argmax(axis=1) # this is where the error is
        ...
        #remove tab to undo edit
        
    else: #johns edit and tabbed belwo
        
        fg_rois_per_this_image = 0.0
        fg_inds = np.array([],dtype=int)
        bg_inds = np.array(range(len(all_rois)))
        labels = np.zeros(len(bg_inds))
    ...
    ############# this should be dependent on if gt is empty ############
    if len(gt_boxes_r) > 0: # johns edit added tab too
    ...
    else: # johns edit added line below also
        bbox_targets_r = np.zeros((len(rois), 5 * num_classes), dtype=np.float32)
    #####################################################################
