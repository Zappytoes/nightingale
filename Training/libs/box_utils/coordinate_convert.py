...
def fblr2corners(BBOX):
    '''
    Converts a diamond annotation style for cv object labeling to a quadrilateral 4 corners annotation sytle
    
    BBOX : array like : front, back, left, right object annotations formatted as [xF,yF,xB,yB,xL,yL,xR,yR]
    
    returns : Four corner points clockwise from front left: xLF,yLF,xRF,yRF,xRB,yRB,xLB,yLB
    '''
    # AFRL box conversion
    # The FRONT Point
    xF = BBOX[0]
    ...
    if not mFB == np.inf and not mLR == np.inf:
        xLF = (bF - bL) / (mFB - mLR)
        xRF = (bF - bR) / (mFB - mLR)
        xRB = (bB - bR) / (mFB - mLR)
        xLB = (bB - bL) / (mFB - mLR)
    ...
    return xLF,yLF,xRF,yRF,xRB,yRB,xLB,yLB
