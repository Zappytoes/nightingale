...
def next_batch(dataset_name, batch_size, shortside_len, is_training):
    ...
    #valid_dataset= ['PRIORI3', 'DOTA1.5', 'ICDAR2015', 'pascal', 'coco', 'bdd100k', 'DOTA', 'HRSC2016', 'UCAS-AOD']
    #if dataset_name not in valid_dataset:
    #    raise ValueError('dataSet name must be in {}'.format(valid_dataset))

    if is_training:
        pattern = os.path.join(cfgs.TFRECORD_PATH, dataset_name + '*_train*')
        #pattern = os.path.join(cfgs.TFRECORD_PATH, cfgs.TFRECORD_NAME)
    #else:
    #    pattern = os.path.join(cfgs.TFRECORD_PATH, dataset_name + '_test*')
